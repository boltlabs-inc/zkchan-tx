...
pub fn create_transaction_input(
    ...
    let utxo_input = transactions::btc::form_transaction_input::<Testnet>(
        &utxo,
        &address,
        &utxo_pk
    )
...
pub fn customer_sign_dual_escrow_transaction(
    ...
    if ret_cust_bal != 0 && cust_change_pk.is_none() {
        return Err(format!("Did not specify a change PK for change: {}", ret_cust_bal))
    ...
    let (tx_preimage, cust_tx) = transactions::btc::form_dual_escrow_transaction(
        &inputs,
        0,
        &musig_output,
        &change_outputs,
    ...
    let cust_signature =
        generate_signature_for_multi_sig_transaction::<Testnet>(&tx_preimage, &private_key).unwrap();

    // encode the public key
    let cust_pubkey =
        encode_public_key_for_transaction::<Testnet>(address_format, &private_key);

    let escrow_tx = EscrowTx { txid_be: txid_buf_be, txid_le: txid_buf_le, prevout: hash_prevout, 
        cust_signature: cust_signature, cust_pubkey: cust_pubkey };
...
pub fn merchant_sign_dual_escrow_transaction(
    ...
    if !verify(&msg, &sig, &witness_pk) {
        return Err(format!("Could not validate signature on <cust-tx-preimage> for dual-funded escrow tx"));
    }
...
mod tests {
    ...
    fn form_dual_escrow_transaction() {
        ...
        .unwrap();
        let merch_input_sk = hex::decode("1a1971e1379beec67178509e25b6772c66cb67bb04d70df2b4bcdb8c08a00827").unwrap();
        let merch_input_pk = merch_private_key.to_public_key().to_secp256k1_public_key().serialize_compressed().to_vec();
        assert_eq!(merch_input_pk, hex::decode("03af0530f244a154b278b34de709b84bb85bb39ff3f1302fc51ae275e5a45fb353").unwrap());

        let merch_utxo = txutil::create_transaction_input(
            &merch_input,
            &merch_input_pk
        )
        .unwrap();
        ...
        println!("Encoded Signature: {}", hex::encode(&escrow_tx.cust_signature));
